...
public protocol CommonSaveDatas: NSObject {
...
extension CommonSaveDatas {
    ...
    private var _savgeDataDidChange: BehaviorRelay<SafeArray<SaveData>> {
        if let dc = objc_getAssociatedObject(self, &savgeDataDidChangeKey) as? BehaviorRelay<SafeArray<SaveData>> {
            return dc
        } else {
            let dc = BehaviorRelay<SafeArray<SaveData>>(value: SafeArray(getDatas()))
...
//
    func savePath() -> URL? {
        let fm = FileManager()
        var fullPath = Paths.Documents + "/CommonSaveDatas"
...
//            lock.unlock()
//        }
        _savgeDataDidChange.accept(SafeArray(datas))
//        lock.lock()
