...
open class SSView: UIView {
    public var viewModel: SSViewModel?
    ...
    open func bindViewModel(viewModel: SSViewModel) {
...
public extension Reactive where Base: SSView {

    var clearLoading: Binder<Bool> {
        return Binder(self.base) { view, attr in
            if attr {
                (view._toastSuperView ?? SSApp.mainWindow)?.showLoadingTextHUD(maskType: .clear)
            }else{
                (view._toastSuperView ?? SSApp.mainWindow)?.hideHUD()
            }
        }
    }
    ...
    var customLoading: Binder<(Bool, String?, Bool)> {
        return Binder(self.base) { view, attr in
            ...
            if isShow {
                if isCanNotTouch {
                    (view._toastSuperView ?? SSApp.mainWindow)?.showLoadingTextHUD(maskType: .clear, message)
                } else {
...
open class TopRoundedCornerView: SSView {
...
open class AllRoundedCornerView: SSView {
