...
//  SSUtils
//
//  Created by yangsq on 2022/9/7.
//

import Foundation
/// A thread-safe array.
public class SSSafeArray<Element> {
    private let queue = DispatchQueue(label: "io.SafeArray", attributes: .concurrent)
...
// MARK: - Properties
public extension SSSafeArray {
    
    var safeArray: [Element] {
        var result: [Element]?
...
public extension SSSafeArray {
    ...
    func filter(_ isIncluded: @escaping (Element) -> Bool) -> SSSafeArray {
        var result: SSSafeArray?
        queue.sync { result = SSSafeArray(self.array.filter(isIncluded)) }
        return result!
    }
    ...
    func sorted(by areInIncreasingOrder: (Element, Element) -> Bool) -> SSSafeArray {
        var result: SSSafeArray?
        queue.sync { result = SSSafeArray(self.array.sorted(by: areInIncreasingOrder)) }
        return result!
    }
...
public extension SSSafeArray {
    ...
    func removeAll(where shouldBeRemoved: @escaping(Element) throws -> Bool) {
        queue.async(flags: .barrier) {
...
// MARK: - Equatable
public extension SSSafeArray where Element: Equatable {
...
public extension SSSafeArray {
    ...
    static func +=(left: inout SSSafeArray, right: Element) {
        left.append(right)
    }
