...
class  ImageBrowseAnimation: NSObject, SSAlertAnimation {
    ...
    func showAnimationOfAnimationView(animationView: SSAlertView, viewSize: CGSize, animated: Bool, completion: (((Bool) -> Void))?) {
        ...
        let browseView = animationView.customView as? SSImageBrowseView
...
public struct ImageBrowse {
    ...
    public weak var fromImageV: UIImageView?
...
public class ImageBrowseCellVM: NSObject {
    ...
    func calculateImageViewSize(size: CGSize) -> CGSize {
        ...
        var width = min(SSApp.width, size.width)
...
class ImageBrowseLayout: UICollectionViewFlowLayout, UIGestureRecognizerDelegate {
    ...
    lazy var longPress: UIPanGestureRecognizer = {
        let longPress = UIPanGestureRecognizer(target: self, action: #selector(longPress(pan:)))
        return longPress
    }()
    ...
    override func prepare() {
        ...
        itemSize = CGSize(width: SSApp.width, height: SSApp.height)
        sectionInset = UIEdgeInsets(top: 0, left: 15.wScale / 2, bottom: 0, right: 15.wScale / 2)
        collectionView.addGestureRecognizer(longPress)
        collectionView.addGestureRecognizer(shareLongPress)
        collectionView.panGestureRecognizer.require(toFail: longPress)
...
//                let url = cell?.cellVM?.browse.largeImageUrl ?? cell?.cellVM?.browse.thumbImageUrl
                ...
                if let image = image {
                    ...
                    activityVC!.completionWithItemsHandler = {
                        ...
                        if completed  {
                            SSApp.mainWindow?.showTextHUD("Success")
                        }
                    }
                }
    ...
    @objc func longPress(pan: UILongPressGestureRecognizer) {
        ...
        let point = pan.location(in: pan.view?.superview)
        let collectionPoint = pan.location(in: pan.view)
...
//            return true
//        }
        if let point = (gestureRecognizer as? UIPanGestureRecognizer)?.translation(in: gestureRecognizer.view), point.y > 0 {
            return true
        }
       return false
    }
...
class ImageBrowseCell: UICollectionViewCell, UIScrollViewDelegate, SSEventTrigger {
    enum Event {
...
//        imageV.autoresizingMask = [.flexibleWidth, .flexibleHeight]
    ...
    override func layoutSubviews() {
        ...
        if !isEndDragAnimation {
            imageV.ss_center = CGPoint(x: max(SSApp.width, scrollView.contentSize.width) / 2, y: scrollView.contentSize.height / 2)
    ...
    func setInitialFrame(size: CGSize?, isHidden: Bool = false) {
        ...
        if size.height > SSApp.height && !isHidden {
            self.scrollView.contentSize = CGSize(width: SSApp.width, height: size.height)
        } else {
            self.scrollView.contentSize = CGSize(width: SSApp.width, height: SSApp.height)
        }
        if isHidden {
            self.imageV.ss_size = CGSize(width: SSApp.width, height:  min(size.height, SSApp.height))
        } else {
            self.imageV.ss_size = CGSize(width: SSApp.width, height:  size.height)
        }
        self.layoutIfNeeded()
    }
...
open class SSImageBrowseView: UIView, SSEventTrigger, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    public init(browses: [ImageBrowse], currentIndex: Int, downloadCompletion: ((String, UIImage?) -> Void)? = nil ) {
        ...
        collectionView.snp.makeConstraints { make in
            make.leading.equalTo(-15.wScale / 2)
            make.top.bottom.equalTo(0)
            make.width.equalTo(self.ss_w + 15.wScale)
        }
        pageControl.snp.makeConstraints { make in
            make.bottom.equalTo(-(SSApp.safeAreaInsets.bottom + 30.wScale))
    ...
    func refreshStartData(isStartAnimation: Bool = false) {
        ...
        let imgeV = cell.imageV
        let imgeVOrigin = CGPoint(x: max(0, (SSApp.width - imgeV.ss_size.width) / 2), y: max(0, (SSApp.height - imgeV.ss_size.height) / 2))
        orginalImageFrame = CGRect(origin: imgeVOrigin, size: imgeV.ss_size)
...
public extension SSImageBrowseView {
    
    static func browse(_ browes: [ImageBrowse], currentIndex: Int, downloadCompletion:  ((String, UIImage?) -> Void)? = nil) {
        if let currentVC = SSApp.mainWindow?.rootViewController {
            let browseView = SSImageBrowseView(browses: browes, currentIndex: currentIndex, downloadCompletion: downloadCompletion)
            let alertView = SSAlertView(customView: browseView, fromViewController: currentVC, animation: ImageBrowseAnimation(), navigationControllerClass: SSNavigationViewController.self, maskType: .none, canPanDimiss: false)
