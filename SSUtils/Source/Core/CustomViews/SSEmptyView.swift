...
open class SSEmptyView: UIView {
    ...
    public lazy var button: SSGradientButton = {
        let button = SSGradientButton.makeButton(type:.system)
    ...
    public lazy var titleLabel: UILabel = {
        let label = UILabel.makeLabel(text: text,
                                      numberOfLines: 0,
                                      alignment: .center)
        label.preferredMaxLayoutWidth = SSApp.width - 120.wScale
        return label
    }()
    ...
    private func layoutEmptyView() {
        if let contenView = contenView {
            let size = contenView.systemLayoutSizeFitting(CGSize(width: SSApp.width, height: CGFloat(MAXFLOAT)))
            self.ss_size = size
    ...
    public func refreshLayout() {
        ...
        if let text = text {
            ...
            titleLabel.snp.remakeConstraints { (make) in
                make.top.equalTo(topMargin)
                make.centerX.equalToSuperview()
                make.width.equalTo(SSApp.width - 120.wScale)
            }
            let labelHeight = titleLabel.systemLayoutSizeFitting(CGSize(width: SSApp.width - 120.wScale, height: CGFloat(MAXFLOAT))).height
        ...
        contenView?.snp.remakeConstraints { (make) in
            make.width.equalTo(SSApp.width)
...
public extension UIView {
    ...
    var notNetworkEmptyView: SSEmptyView? {
        objc_getAssociatedObject(self, &networkErrorEmptyView) as? SSEmptyView
    }
    
    @discardableResult func showNetworkErrorEmptyView( retry: (() -> Void)? = nil ) -> SSEmptyView? {
        if let emptyView = objc_getAssociatedObject(self, &networkErrorEmptyView) as? SSEmptyView {
            return emptyView
        } else {
            let image = SSApp.emptyNotNetworkImage ?? ssImage("notNetwork")
            let text = SSApp.emptyNotNetworkText ?? localized(name: "noInternetAccess")
            let emptyView = SSEmptyView(image: image,
                                      text: text,
                                      textFont: SSApp.emptyTitleFont ?? UIFont.systemFont(ofSize: 16),
                                      textColor: SSApp.emptyTitleColor ?? UIColor.hex(0xcccccc),
                                      buttonTitle: SSApp.emptyNotNetworkButtonCustomView?() == nil ? localized(name: "refresh") : nil,
                                      buttonTitleFont: SSApp.emptyButtonTitleFont ?? UIFont.systemFont(ofSize: 16),
                                      buttonTitleColor: SSApp.emptyButtonTitleColor ?? UIColor.hex(0xcccccc),
                                      buttonCustomView: SSApp.emptyNotNetworkButtonCustomView?() ?? nil,
                                      textTopMargin: SSApp.emptyTitleTopMargin,
                                      buttonTopMargin: SSApp.emptyButtonTopMargin,
                                      buttonTrigger: retry)
            ...
            if let originalColor = originalColor  {
                ...
                backgroundColor = SSApp.emptyBgColor
            }
    ...
    @discardableResult func showEmptyView(image: UIImage? = nil,
                                          ...
                                          buttonCustomView: UIView? = nil,
                                          buttonTrigger: (() -> Void)? = nil) -> SSEmptyView? {
        if let emptyView = objc_getAssociatedObject(self, &emptyViewKey) as? SSEmptyView {
            return emptyView
        } else {
            let emptyView = SSEmptyView(image: image,
                                      text: title,
                                      textFont: titleFont ?? SSApp.emptyTitleFont,
                                      textColor: titleColor ?? SSApp.emptyTitleColor,
                                      buttonTitle: buttonTitle,
                                      buttonTitleFont: buttonTitleFont ?? SSApp.emptyButtonTitleFont,
                                      buttonTitleColor: buttonTitleColor ?? SSApp.emptyButtonTitleColor,
    ...
    var currentEmptyView: SSEmptyView? {
        return objc_getAssociatedObject(self, &emptyViewKey) as? SSEmptyView
    }
