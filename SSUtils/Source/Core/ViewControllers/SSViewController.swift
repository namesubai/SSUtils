...
open class SSViewController: UIViewController,Navigatale {
    open var disposeBag = DisposeBag()
    open var viewModel: SSViewModel?
    open var navigator: SSNavigator?
    ...
    open var navigationBarColor: UIColor? = SSColors.navBarBackgroud
    ...
    public var isTranslucent = SSApp.navIsTranslucent
    ...
    public var backButon: SSCustomButton? {
        self.backButton
    }
    ...
    public init(viewModel: SSViewModel? = nil, navigator: SSNavigator? = nil) {
        self.viewModel = viewModel
        self.navigator = navigator
        super.init(nibName: nil, bundle: nil)
    }
    
    private var backButton: SSCustomButton = {
        let backButton = SSCustomButton(type: .system)
        return backButton
    }()
    ...
    open override func viewDidLoad() {
        ...
        view.backgroundColor = SSColors.backgroud
        
        let backImage: UIImage? = SSApp.navBackImage ?? ssImage("back")
...
//        }
        ...
        let previousVC = self.navigationController?.viewControllers.last as? SSViewController
...
//        }
    ...
    func refreshBackButton() {
        if let image = backImage?.withRenderingMode(.alwaysOriginal) {
            ...
            backButton.contentSize = CGSize(width: 40, height: SSApp.navBarHeight)
...
//            }
    ...
    open func bind() {
        ...
        viewModel.error.asObservable().observe(on: MainScheduler.instance).subscribe(onNext: {
           [weak self] (error) in
            guard let self = self else {return}
            if let error = error as? SSServiceError {
...
public extension Reactive where Base: SSViewController {

    var cannotClickLoading: Binder<Bool> {
        return Binder(self.base) { viewController, attr in
            if attr {
                (viewController.customToastOnView ?? SSApp.mainWindow)?.showLoadingTextHUD(maskType: .clear, tag: 13222)
            } else {
                (viewController.customToastOnView ?? SSApp.mainWindow)?.hideHUD(tag: 13222)
            }
        }
    }
    ...
    var customLoading: Binder<(Bool, String?, Bool)> {
        return Binder(self.base) { viewController, attr in
            ...
            if isShow {
                if isCanNotTouch {
                    viewController.currentCustomToastView = (viewController.customToastOnView ?? SSApp.mainWindow)?.showLoadingTextHUD(maskType: .clear, message,  tag: 13223)
                } else {
                ...
                if isCanNotTouch {
                    (viewController.customToastOnView ?? SSApp.mainWindow)?.hideHUD(tag: 13223)
                } else {
