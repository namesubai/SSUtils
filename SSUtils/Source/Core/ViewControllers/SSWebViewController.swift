...
public class SSCustomWebView: WKWebView {
    deinit {
...
open class SSWebViewController: SSViewController {
    ...
    public lazy var webView: SSCustomWebView = {
        let config = WKWebViewConfiguration()
        config.allowsInlineMediaPlayback = true
        let view = SSCustomWebView(frame: .zero, configuration: config)
        view.navigationDelegate = self
        view.uiDelegate = self
        return view
    }()
    
    private lazy var progressView: SSGradientView = {
        let view = SSGradientView()
        return view
    }()
    
    private lazy var closeButtonItem: UIBarButtonItem = {
        let button = UIBarButtonItem(image: UIImage(systemName: "xmark"), style: .plain, target: self, action: #selector(closeAction))
        button.tintColor = SSColors.headline
        return button
    }()
 ...
 extension SSWebViewController: WKNavigationDelegate {
...
//        self.view.hideEmptyView()
        ...
        if let error = error as? NSError {
            if error.code == NSURLErrorNetworkConnectionLost ||
                ...
                emptyView?.centerOffset = emptyCenterOffset ?? SSApp.emptyCenterOffset
                
            } else {
                self.webView.isHidden = true
                self.view.hideEmptyView()
                let emptyView = self.view.showEmptyView(image: SSApp.emptyErrorImage,
                                                        title: localized(name: "web_load_failed"),
                                                        buttonCustomView: SSApp.emptyNotNetworkButtonCustomView?()) {
                    [weak self] in guard let self = self else { return }
                    if let url = URL(string: self.url ?? "") {
                        self.webView.load(URLRequest(url: url))
                        self.webView.isHidden = false
...
extension SSWebViewController: WKUIDelegate {
