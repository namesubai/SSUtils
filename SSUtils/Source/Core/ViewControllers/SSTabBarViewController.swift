...
open class SSTabBarViewController: UITabBarController,Navigatale {
    open var disposeBag = DisposeBag()
    public private(set) var viewModel: SSViewModel?
    ...
    public var customTabBar: SSTabbar {
        return self.tabBar as! SSTabbar
    }
...
//        self.tabBar.shadowImage = UIImage(named: R.image.tabbar_shadow.name)
        ...
        delegate = self
        self.setValue(SSTabbar(), forKey: "tabBar")
        updateTabBarAppearance(bgColor: SSColors.tabBarBackgroud)
...
extension SSTabBarViewController: UITabBarControllerDelegate {
    ...
    func showTabbar(animated: Bool = true) {
        ...
        if animated {
            UIView.animate(withDuration: 0.25) {
                self.tabBar.ss_y = SSApp.height - self.tabBar.ss_h
            }
        } else {
            self.tabBar.ss_y = SSApp.height - self.tabBar.ss_h
        }
...
public extension Reactive where Base: SSTabBarViewController {

    var cannotClickLoading: Binder<Bool> {
        return Binder(self.base) { viewController, attr in
            if attr {
                (viewController.customToastOnView ?? SSApp.mainWindow)?.showLoadingTextHUD(maskType: .clear)
            }else{
                (viewController.customToastOnView ?? SSApp.mainWindow)?.hideHUD()
            }
        }
    }
    ...
    var customLoading: Binder<(Bool, String?, Bool)> {
        return Binder(self.base) { viewController, attr in
            ...
            if isShow {
                if isCanNotTouch {
                    (viewController.customToastOnView ?? SSApp.mainWindow)?.showLoadingTextHUD(maskType: .clear, message)
                } else {
